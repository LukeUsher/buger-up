...
auto PatchEngine::InjectCall(uintptr_t target, void* detour) -> void {
	...
	VirtualProtect(reinterpret_cast<void*>(target), 5, PAGE_READWRITE, &oldProtect);
	auto FuncBytes = reinterpret_cast<uint8_t*>(target);

	// Automatically NOP leftover bytes if we are overwriting FF 15, allowing replacing indirect calls with direct calls
	if (FuncBytes[0] == 0xFF && FuncBytes[1] == 0x15) FuncBytes[5] = 0x90;

	FuncBytes[0] = 0xE8;
	*reinterpret_cast<uint32_t*>(&FuncBytes[1]) = reinterpret_cast<uint32_t>(detour) - static_cast<uint32_t>(target) - 5;
...
auto PatchEngine::InjectJump(uintptr_t target, void* detour) -> void {
	...
	FuncBytes[0] = 0xE9;
	*reinterpret_cast<uint32_t*>(&FuncBytes[1]) = reinterpret_cast<uint32_t>(detour) - static_cast<uint32_t>(target) - 5;
	VirtualProtect(reinterpret_cast<void*>(target), 5, oldProtect, &tmp);
}
...
auto PatchEngine::LocateFunction(const std::string& library, const std::string function) -> uintptr_t {
	auto hModule = GetModuleHandleA(library.c_str());
	if (hModule == NULL) {
		hModule = LoadLibraryA(library.c_str());
		if (hModule == NULL) return 0;
	}

	auto pFunction = GetProcAddress(hModule, function.c_str());
	if (pFunction == NULL) {
		return 0;
	}
